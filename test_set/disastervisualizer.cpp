    ...
    void drawRoads(const Geometry& geo,
        ...
        for (const string& source: network.network) {
            for (const string& dest: network.network[source]) {
                 ...
                 */
                GLine *line = new GLine(logicalToPhysical(network.cityLocations[source], geo).getX(), logicalToPhysical(network.cityLocations[source], geo).getY(), logicalToPhysical(network.cityLocations[dest], geo).getX(), logicalToPhysical(network.cityLocations[dest], geo).getY());
